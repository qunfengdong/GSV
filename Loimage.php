...
$finalqueryvalue = mysql_real_escape_string($_GET['finalqueryvalue'],$link);
...
function showimagearrow() {
require_once('createarrow.php');

global $annotationid1,$annotationid2,$org1,$org2,$session_id,$session_id_gene,$session_id_gene2,$width,$id_query,$id_ref,$hs19_position,$ce19_position,$annotationidimage,$annotationid2image,$polygon,$xyplotheight,$get_track_id,$imagebgc,$colorchanged,$database_name,$link,$finalqueryvalue,$trackshapechanged,$imagechanged;
...
$image_bg = imagecolorallocate($img, 240, 255, 240);
...
$watermark_black = imagecolorallocatealpha($img, 0, 0, 0,70);    ##For watermark on the lowerimage
...
//$sort_end = 60000;
//$sort_end = 60000000;
$sort_end = $finalqueryvalue;
$sort_value = array(8.5e-13,0.005);
$plot_length = $sort_end - $sort_start;
$plot_value_height = $sort_value[1] - $sort_value[0];
...
$hs19ce19start = array($id_query);


$startandend = array($ce19_position);
...
($org1 == $gene[$d])? $arrowpos -= 10 : $arrowpos -= 10;
...
$accdown = "arrow";
$WaterMarkText = $acc_track_name."_".$org2;
...
$WaterMarkText = $acc_track_name."_".$org2;
imagestring($img, 2, 5, 45, $WaterMarkText, $watermark_black);
$bar_start = floor((($ans[0]-$id_query)/$query_length)*$width);
$bar_end = floor((($ans[1]-$id_query)/$query_length)*$width);
...
mysql_close($link);
ImagePNG($img);
imagepng($img, "syn/synteny_annotation_{$session_id_gene}/{$acc_track_name}_{$org2}_{$session_id_gene}_image_{$imagechanged}.png");
imagedestroy($img);
